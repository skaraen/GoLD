...
func (inv *Invocation) TranslateToLLVMStack(currBlk *cfg.Block, exitBlk *cfg.Block, llvmProgram *llvm.LLVMProgram, funcEntry *st.FuncEntry, tables *st.SymbolTables) *cfg.Block {
	...
	dest := llvm.NewLLVMRegister(llvmProgram.GenerateRegisterName(), st.NewVarEntry(inv.String(), invEntry.RetTy, st.LOCAL, inv.Token))
	invInstn := llvm.NewInvInstn(invEntry, dest, argList)
